...
<template>
  <div class="flex items-center gap-1 bg-muted/50 rounded-full border border-border/50" :class="size === 'sm' ? 'p-1' : 'p-1.5 rounded-2xl'">
    <button
      v-for="mode in modes"
      :key="mode.value"
      ...
      :class="[
        ...
        size === 'sm' ? 'p-1.5 rounded-full' : 'p-2.5 rounded-xl'
      ]"
      :title="mode.label"
    >
      <component :is="mode.icon" :class="size === 'sm' ? 'h-3.5 w-3.5' : 'h-5 w-5'" />
    </button>
  </div>
</template>
