       ...
       <div class="container max-w-5xl -mt-12 relative z-10 px-4 mx-auto">
          ...
          <div class="bg-card rounded-[2.5rem] shadow-2xl p-8 md:p-16 mb-12 border border-border ring-1 ring-border/5">
              ...
              <div class="border-t border-border mt-16 pt-10 flex flex-col md:flex-row items-center justify-between gap-6 px-2">
                 ...
                 <RouterLink to="/news" class="text-primary hover:opacity-70 font-black uppercase tracking-widest text-[10px] flex items-center gap-3 bg-muted/50 px-6 py-3 rounded-2xl border border-border transition-all hover:gap-4">
...
const fetchNewsById = async (id: string) => {
    ...
    try {
        ...
        if (snap.exists()) {
            newsItem.value = { id: snap.id, ...snap.data() }
        }
    } catch (e) {
        console.error(e)
    } finally {
