<template>
  <div class="container py-10 space-y-10 transition-colors duration-300">
    ...
    <!-- Users Table -->
    <div
      class="rounded-3xl border border-border bg-card shadow-sm overflow-x-auto ring-1 ring-border/5"
    >
        ...
        <Table>
          ...
          <TableBody>
            <TableRow
              v-for="u in users"
              :key="u.uid"
              class="hover:bg-muted/50 transition-colors border-b-border last:border-0 group"
            >
              <TableCell class="py-5 pl-8 text-left">
                <div class="flex items-center gap-4">
                  <Avatar
                    class="h-11 w-11 rounded-xl border-2 border-background shadow-sm ring-1 ring-border"
                  >
                  ...
                  <div class="flex flex-col gap-0.5 text-left">
                    <span
                      class="font-extrabold text-foreground flex items-center gap-2"
                    >
                      {{ u.displayName }}
                      <span
              ...
              <TableCell class="py-5 text-left">
                <Badge
                  ...
                  :class="{
                    'bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-500/20 ring-1 ring-blue-500/20':
                      u.role === 'admin',
                    'bg-rose-500/10 text-rose-600 dark:text-rose-400 hover:bg-rose-500/20 ring-1 ring-rose-500/20':
                      u.role === 'super_admin',
              ...
              <TableCell class="text-right py-5 pr-8">
                <div
                  v-if="u.role !== 'super_admin' || authStore.isSuperAdmin"
                  class="flex justify-end gap-2"
                >
                  <Button
                    variant="ghost"
                    size="icon"
                    class="h-8 w-8 rounded-lg hover:bg-muted transition-colors text-muted-foreground"
                    @click="openEditProfile(u)"
                  >
                  ...
                  <DropdownMenu v-if="u.uid !== authStore.user?.uid">
                    <DropdownMenuTrigger as-child>
                      <Button
                        variant="ghost"
                        size="icon"
                        class="h-8 w-8 p-0 rounded-lg hover:bg-muted transition-colors text-muted-foreground"
                      >
                ...
                <div v-else class="flex justify-end">
                  <Badge
                    variant="outline"
                    class="text-[8px] opacity-30 border-muted-foreground"
                    >PROTECTED</Badge
                  >
                </div>
              </TableCell>
    ...
    <Dialog v-model:open="showEditModal">
      <DialogContent
        class="max-w-xl rounded-2xl p-0 border-none bg-card shadow-lg"
      >
        <div class="p-8 space-y-8">
          <DialogHeader>
            <DialogTitle
              class="text-xl font-semibold text-foreground font-khmer"
              >{{ $t("admin.users.edit_profile") }}</DialogTitle
            >
            <DialogDescription class="font-normal text-muted-foreground">{{
              $t("admin.users.edit_profile_description")
            }}</DialogDescription>
          </DialogHeader>
          <div v-if="editingUser" class="space-y-6">
            <div class="grid grid-cols-2 gap-6">
              <div class="grid gap-2">
                <Label
                  ...
                  >{{ $t("admin.users.table_name") }}</Label
                >
                <Input
                  v-model="editForm.displayName"
                  class="rounded-xl border-border bg-muted/30 font-normal focus:bg-background transition-all h-11"
                />
              </div>
              <div class="grid gap-2">
                <Label
                  class="text-xs font-medium uppercase tracking-wider text-muted-foreground/80"
                  >{{ $t("admin.users.table_phone") }}</Label
                >
                <Input
            ...
            <div class="grid gap-2">
              ...
              >
              <Input
                v-model="editForm.telegramHandle"
            ...
            <DialogFooter class="flex flex-row gap-3 pt-4">
              <Button
                variant="outline"
                class="flex-1 rounded-xl h-11 font-medium border-border text-muted-foreground hover:bg-muted bg-background/50"
                @click="showEditModal = false"
                >{{ $t("admin.forms.cancel") }}</Button
              >
              <Button
                class="flex-1 rounded-xl h-11 font-medium bg-primary text-white hover:bg-primary/90 shadow-sm"
    ...
    <Dialog v-model:open="showAddUserModal">
      <DialogContent
        class="max-w-md rounded-2xl p-8 border-none bg-card shadow-lg"
      >
        <DialogHeader class="space-y-4 text-center items-center">
          <div
            class="size-14 rounded-2xl bg-orange-500/5 flex items-center justify-center text-orange-500 border border-orange-500/10"
          >
            <ShieldAlertIcon class="size-7" />
          </div>
          <DialogTitle class="text-xl font-semibold text-foreground font-khmer"
            >បន្ថែមបុគ្គលិកថ្មី</DialogTitle
          >
          <DialogDescription
            class="font-normal text-muted-foreground text-center leading-relaxed"
          >
            ចំណាំ៖ អ្នកប្រើប្រាស់ដែលបន្ថែមនៅទីនេះ
            ត្រូវតែចុះឈ្មោះចូលប្រើប្រាស់ជាមុនសិន ឬប្រើប្រាស់ Cloud Functions
            ដើម្បីបង្កើត Auth User ។
          </DialogDescription>
        </DialogHeader>
        <div class="space-y-4 text-center pt-4">
          <p class="text-sm font-normal text-muted-foreground">
            ក្នុងកំណែបច្ចុប្បន្ន សូមអោយបុគ្គលិកចុះឈ្មោះដោយខ្លួនឯងជាមុនសិន។
            បន្ទាប់មក Admin អាចមកកំណត់តួនាទីនៅទីនេះបាន។
          </p>
          <DialogFooter class="pt-8">
            <Button
              variant="outline"
              class="w-full rounded-xl h-11 font-medium border-border bg-background/50 hover:bg-muted"
